...
solveEasy crates moves = Just $ map head $ makeMoves performMove crates moves
  where
    ...
    performMove crates move =
      let (toMove, left) = splitAt (amount move) (crates !! from move)
...
solveHard crates moves = Just $ map head $ makeMoves performMove crates moves
  where
    performMove :: [[Char]] -> Move -> [[Char]]
    performMove crates move =
      let (toMove, left) = splitAt (amount move) (crates !! (from move - 1))
...
makeMoves performMove crates moves =
  foldl performMove crates $
...
testsEasy =
  Tests
    { run = runProblemString (Problem parseEasy (uncurry solveEasy) id),
      inputs = ["    [D]    \n[N] [C]    \n[Z] [M] [P]\n 1   2   3 \n\nmove 1 from 2 to 1\nmove 3 from 1 to 3\nmove 2 from 2 to 1\nmove 1 from 1 to 2"],
      expectedOutputs = ["CMZ"]
    }
